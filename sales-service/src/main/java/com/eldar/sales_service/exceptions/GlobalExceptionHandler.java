package com.eldar.sales_service.exceptions;

import com.eldar.sales_service.exceptions.customs.BadCredentialssException;
...
public class GlobalExceptionHandler {
    ...
    public ResponseEntity<ErrorResponse> processExternalServiceException(FeignException ex, HttpServletRequest request) {
       String message = ex.contentUTF8().replaceAll(".*\"message\":\"(.*?)\".*", "$1");
        ErrorResponse errorResponse = ErrorResponse.builder()
                .code(ex.status())
                .status(HttpStatus.valueOf(ex.status()).getReasonPhrase())
                .timestamp(LocalDateTime.now())
                .message(message)
                .path(request.getRequestURI())
                .build();
